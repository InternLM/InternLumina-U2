...
class TokenBridge(nn.Module):
    ...
    def forward_channel_ar(self, spatial_features, channel_targets, mask):
        ...
        spatial_features = spatial_features[mask_indices]  # [num_masked, decoder_embed_dim] (419, 768)
        print(f"spatial_features shape: {spatial_features.shape}")

        channel_targets = channel_targets[mask_indices]    # [num_masked, C] (419, 16)
        print(f"channel_targets shape: {channel_targets.shape}")
        print(f"channel_targets: {channel_targets}")
        # Project decoder features
        cond = self.condition_proj(spatial_features)  # [num_masked, channel_embed_dim] ([419, 768])
        print(f"cond shape: {cond.shape}")
        # Initialize sequence with condition
        channel_sequence = [cond.unsqueeze(1)]  # [num_masked, 1, channel_embed_dim] ([419, 1, 768])
        print(f"channel_sequence shape: {get_list_shape(channel_sequence)}")
        ...
        for i in range(self.vae_embed_dim-1):
            ...
            print(f"channel_embed shape: {channel_embed.shape}") # ([419, 768])
            channel_sequence.append(channel_embed.unsqueeze(1))

        print(f"channel_sequence shape: {get_list_shape(channel_sequence)}") 
        # Stack sequence
        x = torch.cat(channel_sequence, dim=1)  # [num_masked, C, channel_embed_dim] ([419, 16, 768])
        ...
        print(f"x2 shape: {x.shape}") # ([419, 16, 768])
        x = self.channel_norm(x)
        print(f"x3 shape: {x.shape}") # ([419, 16, 768])

        x = self.channel_final(x, cond)
        print(f"x4 shape: {x.shape}") # ([419, 16, 768])
        # Get predictions
        logits = torch.stack([
            head(x[:, i]) for i, head in enumerate(self.channel_heads)
        ], dim=1)  # [num_masked, C, quant_levels]
        print(f"logits shape: {logits.shape}") # ([419, 16, 64])
        
        return logits
    
    def forward_channel_parallel(self, spatial_features, mask, temperature=1.0):
        """
        Parallel prediction of all channel tokens (non-autoregressive).
        
        This function predicts all channels for the masked spatial positions in
        parallel, without using previously predicted channels as input.
        
        Args:
            spatial_features (torch.Tensor): Spatial features from decoder of
                shape [B, N, decoder_embed_dim].
            mask (torch.Tensor): Boolean or {0,1} mask of shape [B, N], where 1
                denotes positions to be predicted.
            temperature (float): Temperature for sampling. If 0, argmax is used.
        
        Returns:
            torch.Tensor: Predicted channel tokens of shape
                [num_masked, vae_embed_dim], in the original channel order.
        """
        # Extract masked positions
        mask_indices = mask.bool()
        spatial_features = spatial_features[mask_indices]  # [num_masked, decoder_embed_dim]
        
        # If no positions are masked, return an empty tensor
        if spatial_features.numel() == 0:
            return torch.empty(
                0, self.vae_embed_dim, dtype=torch.long, device=spatial_features.device
            )
        
        # Project decoder features
        cond = self.condition_proj(spatial_features)  # [num_masked, channel_embed_dim]
        
        # Broadcast condition over channels and add timestep embeddings
        num_masked = cond.shape[0]
        cond_seq = cond.unsqueeze(1).expand(num_masked, self.vae_embed_dim, -1)
        x = cond_seq + self.channel_embed.expand(num_masked, -1, -1)
        cond_seq = cond_seq + self.timesteps_embeddings.expand(num_masked, -1, -1)
        
        # Apply transformer blocks without causal mask (full parallel over channels)
        for block in self.channel_blocks:
            x = block(x, attn_mask=None, c=cond_seq)
        x = self.channel_norm(x)
        x = self.channel_final(x, cond_seq)
        
        # Get logits for all channels
        logits = torch.stack(
            [head(x[:, i]) for i, head in enumerate(self.channel_heads)],
            dim=1
        )  # [num_masked, C, quant_levels]
        
        # Sample tokens per channel
        if temperature == 0:
            tokens = logits.argmax(dim=-1)
        else:
            probs = F.softmax(logits / temperature, dim=-1)
            tokens = torch.multinomial(
                probs.view(-1, self.quant_levels), num_samples=1
            ).view(-1, self.vae_embed_dim)
    ...
    def sample_tokens_parallel(self, bsz, num_iter=64, cfg=1.0, cfg_schedule="linear",
                               labels=None, temperature=1.0):
        """
        Sample tokens using iterative masked prediction with parallel channel decoding.
        
        This variant replaces the channel-wise autoregressive decoder with a
        fully parallel decoder over channels.
        
        Args:
            bsz (int): Batch size.
            num_iter (int): Number of iterative refinement steps.
            cfg (float): Classifier-free guidance scale.
            cfg_schedule (str): Schedule for classifier-free guidance.
            labels (torch.Tensor): Optional class labels.
            temperature (float): Sampling temperature for channel logits.
        
        Returns:
            torch.Tensor: Generated continuous tokens of shape
                [B, vae_embed_dim, H, W].
        """
        device = next(self.parameters()).device
        
        # Initialize tokens and masks in original channel order
        mask = torch.ones(bsz, self.seq_len, device=device)
        tokens = torch.zeros(
            bsz, self.seq_len, self.vae_embed_dim, device=device, dtype=torch.long
        )
        orders = self.sample_orders(bsz)
        ...
        for step in range(num_iter):
            ...
            # Reconstruction values directly from tokens in original order
            cur_values = self.reconstruction_values[cur_tokens]
            cur_continuous = (cur_values + self.std_range) / (2 * self.std_range) * \
                             (self.quant_max - self.quant_min) + self.quant_min
            ...
            # Channel prediction (parallel over channels)
            if cfg != 1.0:
                z_cond_masked = z_cond[mask_to_pred.nonzero(as_tuple=True)]
                z_uncond_masked = z_uncond[mask_to_pred.nonzero(as_tuple=True)]
                ...
                guided_feature = z_uncond_masked + cfg_iter * (z_cond_masked - z_uncond_masked)
                # Use parallel channel head for guided features
                guided_feature_full = torch.zeros_like(z_cond)
                guided_feature_full[mask_to_pred.nonzero(as_tuple=True)] = guided_feature
                next_tokens = self.forward_channel_parallel(
                    guided_feature_full, mask_to_pred, temperature
                )
            else:
                z_masked = z[mask_to_pred.nonzero(as_tuple=True)]
                z_full = torch.zeros_like(z)
                z_full[mask_to_pred.nonzero(as_tuple=True)] = z_masked
                next_tokens = self.forward_channel_parallel(
                    z_full, mask_to_pred, temperature
                )
