...
# Lazy import for gs modules to avoid import errors when gs is not needed
_gs_modules_loaded = False
_hammersley_sequence = None
_Gaussian = None
_GaussianRenderer = None

def _ensure_gs_modules():
    """Ensure gs modules are imported. Raise ImportError if they cannot be imported."""
    global _gs_modules_loaded, _hammersley_sequence, _Gaussian, _GaussianRenderer
    if not _gs_modules_loaded:
        try:
            ...
            _hammersley_sequence = hammersley_sequence
            _Gaussian = Gaussian
            _GaussianRenderer = GaussianRenderer
            _gs_modules_loaded = True
        except ImportError as e:
            raise ImportError(
                f"Failed to import gs modules from 'atoken_inference.model.gs'. "
                f"This is required for SLatGaussianDecoder. "
                f"Please ensure the gs module is available. "
                f"Original error: {e}"
            )
    return _hammersley_sequence, _Gaussian, _GaussianRenderer
...
class SLatGaussianDecoder(SparseTransformerBase):
    ...
    def _build_perturbation(self) -> None:
        hammersley_sequence, _, _ = _ensure_gs_modules()
        perturbation = [
    ...
    def to_representation(self, x: SparseTensor) -> List:
        ...
        """
        _, Gaussian, _ = _ensure_gs_modules()
...
class SLatGaussianDecoderTrain(SLatGaussianDecoder, BaseModule):
    def __init__(self, *args, **kwargs):
        ...
        _, _, GaussianRenderer = _ensure_gs_modules()
