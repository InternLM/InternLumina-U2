...
class ATokenWrapper(nn.Module):
    ...
    def forward(self, images):
        ...
        # Recover per-image spatial sizes from the batched sparse coordinates.
        # Coordinates are [batch, t, h, w, z]; image_sizes is (h, w).
        image_sizes = []
        for batch_idx in range(len(images)):
            batch_coords = x.coords[x.coords[:, 0] == batch_idx]
            if batch_coords.numel() == 0:
                raise ValueError(f"Image {batch_idx} produced no sparse patches")
            image_sizes.append((
                int(batch_coords[:, 2].max().item()) + 1,
                int(batch_coords[:, 3].max().item()) + 1,
            ))
    ...
    def extract_encode(self, x):
        ...
        if self.arch_cfg.get("use_quantizer", False):
            ...
            raise ValueError("Quantizer is not used in the model.")

        return indices, latent.coords

    def extract_decode(self, indices, coords):
        """
        Inference the video and get the rec.
        """
        latent = SparseTensor(
            feats=self.model.quantizer.indices_to_codes(indices),
            coords=coords,
        )
        rec = self.decode(latent, training=False).sample
